...
{
	// S čakanjem v zanki zagotovimo, da je v oddajnem delu USART vmesnika prešnje pošiljanje končano.
	// Za ugotavljanje stanja prejšnjega prenosa uporabimo ustrezno nizko-nivojsko LL funkcijo za delo z USART-om.
...
uint8_t SCI_is_data_waiting(void)
{
	...
	return (uint8_t)LL_USART_IsActiveFlag_RXNE_RXFNE (SCI.USARTx);
